...
* `Remove Branch`: removes the branch currently selected. Note that the button is disabled unless other branch is checkout, as a branch cannot be deleted if it is locally used.
